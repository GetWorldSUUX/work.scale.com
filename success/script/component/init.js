    ...
    let globalSlides = 0;
      ...
      $(".send_mail").click(function(){
        ...
        $.ajax({
          method: "POST",
          url: "server.php",
          data: { status: "mail_subscribe" , mail: val }
        })
        .done(function(data) {
          window.open("/newsletter-subscription-success/", '_blank');
        });
      });
      ...
      $(".btn-send-server").click(function(){
        ...
        $.ajax({
          ...
          console.log(data); 
          if(!data.success && data.error == "nameIsNotValid"){
            form.find("input").removeClass("error");
            $(".popup").find(".perload").fadeOut(300);
            form.find("input[name=name]").addClass("error");
          }else if(!data.success && data.error == "phoneIsNotValid"){
            form.find("input").removeClass("error");
            $(".popup").find(".perload").fadeOut(300);
            ...
            $(".popup").find(".perload").fadeOut(300);
            $(".popup").fadeOut(300);
            window.location.replace("/submitted-form/", "_blank");
